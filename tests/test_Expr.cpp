...
using Program = std::vector<std::unique_ptr<const Stmt>>;
...
struct unique_ptr {
  ...
  const Expr& operator*() { return *wrapped_; }

  const std::unique_ptr<Expr>& operator->() { return wrapped_; }

  std::unique_ptr<Expr> take() { return std::move(wrapped_); }

private:
  std::unique_ptr<Expr> wrapped_;
};

unique_ptr let(int value) {
  return std::make_unique<ConstExpr>(value);
}

unique_ptr var(const char* name) {
  return std::make_unique<VarExpr>(name);
}

unique_ptr operator+(unique_ptr lhs, unique_ptr rhs) {
...
using StateGenerator  = std::tuple<State, Program, Program>;
using VariableBinding = std::pair<StringRef, num_t>;
using StateLiteral    = std::initializer_list<VariableBinding>;

StateGenerator make_state_generator(StateLiteral l) {

  State state(l);

  Program reads;
  reads.reserve(l.size());

  std::transform(l.begin(),
                 l.end(),
                 std::back_inserter(reads),
                 [](const VariableBinding& binding) {
                   return std::make_unique<ReadStmt>(binding.first);
                 });

  Program assignments;
  assignments.reserve(l.size());
  ...
  std::transform(l.begin(),
                 ...
                 [](const VariableBinding& binding) {
                   return std::make_unique<AssignStmt>(
                       binding.first,
                       std::make_unique<ConstExpr>(binding.second));
                 });

  return std::make_tuple(
      std::move(state), std::move(reads), std::move(assignments));
}

template <typename Check>
Error eval(StateGenerator stateGen, std::unique_ptr<Expr> expr, Check check) {

  State& state         = std::get<0>(stateGen);
  Program& reads       = std::get<1>(stateGen);
  Program& assignments = std::get<2>(stateGen);
  ...
  // Testing reads
  {
    ...
    std::transform(state.begin(),
                   state.end(),
                   std::ostream_iterator<num_t>(input, " "),
                   [](const auto& keyValue) { return keyValue.getValue(); });

    reads.push_back(std::make_unique<const WriteStmt>(std::move(expr)));
    ...
    if (output.eof() || output.fail()) {
      return make_error<StringError>("IO error", inconvertibleErrorCode());
    }
    check(result);
  }
  ...
  // Testing assignments
  {
    ...
    assignments.push_back(std::move(reads.back()));
    reads.pop_back();
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 772}, {"x1", 270}, {"x2", 779}});
  auto expr = (((((var("x2")) + (let(264))) * ((var("x0")) * (var("x2")))) -
                (((var("x2")) - (var("x0"))) + ((let(58)) + (var("x1"))))));

  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == 627247349);
  });
  CHECK(!error);
}
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 152}, {"x1", 735}, {"x2", 825}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 233}, {"x1", 682}, {"x2", 165}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 829}, {"x1", 146}, {"x2", 601}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 288}, {"x1", 299}, {"x2", 455}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 986}, {"x1", 185}, {"x2", 163}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 205}, {"x1", 352}, {"x2", 947}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 405}, {"x1", 434}, {"x2", 999}});
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator(
      {{"x0", 697}, {"x1", 245}, {"x2", 438}, {"x3", 652}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 230}, {"x1", 92}, {"x2", 397}, {"x3", 149}});
...
ANON_TEST_CASE() {
  ...
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == -50127689595846096);
  });
  CHECK(!error);
}
...
ANON_TEST_CASE() {
  ...
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == 947588346);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 509},
                                              {"x1", 876},
                                              {"x2", 280},
                                              {"x3", 393},
                                              {"x4", 446},
                                              {"x5", 128}});
  auto expr   = ((((((let(855)) + (var("x4"))) + ((let(215)) + (var("x0")))) +
                 (((let(481)) - (let(776))) - ((var("x4")) - (var("x4"))))) -
                ((((let(712)) + (let(485))) + ((let(971)) * (let(781)))) +
                 (((var("x2")) * (var("x3"))) * ((var("x5")) - (let(996)))))));
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == 94756902);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 561},
                                              {"x1", 549},
                                              {"x2", 417},
                                              {"x3", 639},
                                              {"x4", 507},
                                              {"x5", 439}});
  auto expr   = ((((((var("x1")) * (let(291))) - ((var("x5")) - (let(858)))) -
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 695},
                                              {"x1", 148},
                                              {"x2", 358},
                                              {"x3", 175},
                                              {"x4", 75},
                                              {"x5", 193}});
  auto expr   = ((((((let(331)) - (let(292))) + ((var("x1")) * (var("x5")))) -
                 (((let(310)) * (let(304))) * ((var("x4")) - (var("x4"))))) +
                ((((var("x5")) + (var("x2"))) * ((var("x0")) + (let(732)))) -
                 (((let(396)) * (var("x5"))) * ((var("x3")) * (var("x4")))))));
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == -1002302620);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 858},
                                              {"x1", 997},
                                              {"x2", 883},
                                              {"x3", 987},
                                              {"x4", 121},
                                              {"x5", 53}});
  auto expr   = ((((((let(725)) * (let(587))) + ((let(144)) * (let(91)))) *
                 (((var("x5")) + (let(590))) + ((var("x5")) * (let(618))))) *
                ((((var("x4")) - (var("x2"))) + ((var("x4")) + (var("x4")))) -
                 (((let(931)) - (var("x4"))) - ((var("x1")) * (var("x4")))))));
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == 1747914667703841);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 988},
                                              {"x1", 748},
                                              {"x2", 167},
                                              {"x3", 781},
                                              {"x4", 981},
  ...
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == -21857977456226);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 647},
                                              {"x1", 166},
                                              {"x2", 61},
                                              {"x3", 125},
                                              {"x4", 256},
  ...
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == -6756188561647200);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 190},
                                              {"x1", 823},
                                              {"x2", 226},
                                              {"x3", 530},
                                              {"x4", 417},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 351},
                                              {"x1", 771},
                                              {"x2", 843},
                                              {"x3", 415},
                                              {"x4", 762},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 67},
                                              {"x1", 188},
                                              {"x2", 687},
                                              {"x3", 888},
                                              {"x4", 597},
                                              {"x5", 185}});
  auto expr   = ((((((let(914)) - (var("x5"))) + ((let(139)) * (var("x4")))) *
                 (((var("x5")) * (var("x0"))) + ((let(262)) * (let(822))))) +
                ((((let(748)) - (var("x2"))) - ((var("x4")) * (var("x0")))) *
                 (((var("x1")) + (var("x3"))) + ((let(392)) + (var("x0")))))));
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == 19004856578);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 822},
                                              {"x1", 27},
                                              {"x2", 601},
                                              {"x3", 277},
                                              {"x4", 790},
                                              {"x5", 163}});
  auto expr   = ((((((var("x2")) - (let(326))) * ((var("x4")) - (var("x0")))) -
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 447},
                                              {"x1", 286},
                                              {"x2", 940},
                                              {"x3", 522},
                                              {"x4", 79},
                                              {"x5", 750},
                                              {"x6", 41}});
  auto expr   = (((((((let(473)) - (var("x3"))) + ((let(542)) * (var("x3")))) -
  ...
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == 55605271039900730);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 221},
                                              {"x1", 581},
                                              {"x2", 872},
                                              {"x3", 554},
                                              {"x4", 448},
                                              {"x5", 158},
                                              {"x6", 566}});
  auto expr = (((((((var("x1")) * (var("x5"))) - ((var("x0")) - (var("x2")))) +
  ...
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == 1682611700987842);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 274},
                                              {"x1", 152},
                                              {"x2", 649},
                                              {"x3", 245},
                                              {"x4", 523},
                                              {"x5", 580},
                                              {"x6", 714}});
  auto expr   = (((((((var("x2")) * (var("x2"))) - ((let(383)) * (var("x4")))) *
  ...
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == -3443733562308592);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 37},
                                              {"x1", 905},
                                              {"x2", 599},
                                              {"x3", 116},
                                              {"x4", 494},
                                              {"x5", 253},
                                              {"x6", 499}});
  auto expr   = (((((((let(277)) * (let(741))) + ((let(722)) + (let(866)))) +
  ...
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == 138411798956);
  });
  CHECK(!error);
}

ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 276},
                                              {"x1", 354},
                                              {"x2", 699},
                                              {"x3", 367},
                                              {"x4", 93},
                                              ...
                                              {"x6", 12},
                                              {"x7", 14}});
  auto expr = (((((((var("x0")) - (var("x4"))) + ((var("x2")) - (var("x2")))) *
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 476}, {"x1", 183}, {"x2", 881}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 246}, {"x1", 317}, {"x2", 804}});
  auto expr   = (((((var("x0")) - (let(523))) < ((let(723)) == (let(961)))) *
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 211}, {"x1", 68}, {"x2", 317}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 523}, {"x1", 188}, {"x2", 920}});
  auto expr   = (((((var("x1")) + (let(350))) || ((var("x1")) != (let(634)))) ==
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 877}, {"x1", 434}, {"x2", 278}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 745}, {"x1", 991}, {"x2", 725}});
  auto expr   = (((((let(305)) >= (let(820))) >= ((let(9)) != (let(528)))) !=
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 586}, {"x1", 518}, {"x2", 454}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 875}, {"x1", 938}, {"x2", 833}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 320}, {"x1", 764}, {"x2", 341}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 58}, {"x1", 176}, {"x2", 182}});
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator(
      {{"x0", 176}, {"x1", 207}, {"x2", 710}, {"x3", 873}});
  auto expr   = (((((var("x3")) || (let(564))) || ((let(636)) + (let(543)))) !=
                (((var("x0")) != (var("x1"))) - ((var("x3")) && (var("x1"))))));
  Error error = eval(std::move(stateGenerator), expr.take(), [](num_t result) {
    CHECK(result == 1);
  });
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 277}, {"x1", 241}, {"x2", 91}, {"x3", 259}});
...
ANON_TEST_CASE() {
  auto stateGenerator =
      make_state_generator({{"x0", 502}, {"x1", 31}, {"x2", 230}, {"x3", 288}});
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator(
      {{"x0", 902}, {"x1", 175}, {"x2", 705}, {"x3", 154}});
  auto expr = (((((let(180)) && (var("x1"))) == ((var("x1")) || (var("x0")))) -
...
ANON_TEST_CASE() {
  ...
  auto expr   = ((((((var("x0")) + (let(297))) == ((let(8)) - (var("x3")))) &&
...
ANON_TEST_CASE() {
  ...
  auto expr   = ((((((var("x2")) >= (let(156))) - ((let(752)) && (let(401)))) !=
...
ANON_TEST_CASE() {
  ...
  auto expr = ((((((var("x1")) <= (var("x0"))) >= ((let(174)) == (let(843)))) >
...
ANON_TEST_CASE() {
  ...
  auto expr = ((((((let(144)) >= (var("x3"))) != ((var("x2")) >= (let(668)))) ==
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 197},
                                              {"x1", 986},
                                              {"x2", 414},
                                              {"x3", 476},
                                              {"x4", 175},
                                              {"x5", 468}});
  auto expr   = ((((((var("x3")) && (var("x0"))) != ((var("x3")) + (let(79)))) *
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 315},
                                              {"x1", 607},
                                              {"x2", 122},
                                              {"x3", 908},
                                              {"x4", 139},
                                              {"x5", 6}});
  auto expr   = ((((((var("x4")) + (let(697))) || ((let(914)) >= (let(381)))) +
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 729},
                                              {"x1", 885},
                                              {"x2", 916},
                                              {"x3", 878},
                                              {"x4", 575},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 738},
                                              {"x1", 296},
                                              {"x2", 757},
                                              {"x3", 384},
                                              {"x4", 263},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 217},
                                              {"x1", 279},
                                              {"x2", 695},
                                              {"x3", 451},
                                              {"x4", 530},
                                              {"x5", 71}});
  auto expr   = ((((((let(48)) - (let(772))) + ((var("x5")) * (var("x5")))) ||
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 328},
                                              {"x1", 983},
                                              {"x2", 663},
                                              {"x3", 225},
                                              {"x4", 490},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 809},
                                              {"x1", 472},
                                              {"x2", 254},
                                              {"x3", 342},
                                              {"x4", 715},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 373},
                                              {"x1", 529},
                                              {"x2", 334},
                                              {"x3", 911},
                                              {"x4", 728},
                                              {"x5", 20}});
  auto expr = ((((((let(684)) == (var("x2"))) == ((let(381)) <= (var("x1")))) >=
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 746},
                                              {"x1", 766},
                                              {"x2", 343},
                                              {"x3", 871},
                                              {"x4", 53},
                                              {"x5", 8}});
  auto expr = ((((((var("x4")) && (let(531))) <= ((var("x2")) * (var("x3")))) *
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 639},
                                              {"x1", 876},
                                              {"x2", 27},
                                              {"x3", 192},
                                              {"x4", 138},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 207},
                                              {"x1", 410},
                                              {"x2", 839},
                                              {"x3", 488},
                                              {"x4", 566},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 996},
                                              {"x1", 790},
                                              {"x2", 756},
                                              {"x3", 593},
                                              {"x4", 127},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 195},
                                              {"x1", 321},
                                              {"x2", 808},
                                              {"x3", 951},
                                              {"x4", 74},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 308},
                                              {"x1", 859},
                                              {"x2", 824},
                                              {"x3", 927},
                                              {"x4", 504},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 199},
                                              {"x1", 828},
                                              {"x2", 592},
                                              {"x3", 256},
                                              {"x4", 891},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 318},
                                              {"x1", 395},
                                              {"x2", 485},
                                              {"x3", 857},
                                              {"x4", 299},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 27},
                                              {"x1", 325},
                                              {"x2", 453},
                                              {"x3", 861},
                                              {"x4", 933},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 290},
                                              {"x1", 252},
                                              {"x2", 353},
                                              {"x3", 334},
                                              {"x4", 83},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 371},
                                              {"x1", 966},
                                              {"x2", 726},
                                              {"x3", 574},
                                              {"x4", 138},
...
ANON_TEST_CASE() {
  auto stateGenerator = make_state_generator({{"x0", 959},
                                              {"x1", 997},
                                              {"x2", 989},
                                              {"x3", 236},
                                              {"x4", 386},
